...
const initialState: BuilderState = {
  ...
  // ── Test prefill — remove before production ──────────
  websiteUrl: "https://www.toyota.ca/en/",
  companyName: "Open Road Toyota Richmond",
...
const SI_MAX = 300;
const SI_STORAGE_KEY = "aipb-special-instructions";
const WORKSPACE_KEY = "aipb-workspace-v2";
...
function App() {
  ...
  const [state, setState] = useState<BuilderState>(() => {
    try {
      ...
      if (ws.state) return { ...initialState, ...ws.state };
      // legacy SI key migration
      const si = JSON.parse(localStorage.getItem(SI_STORAGE_KEY) ?? "{}");
      if (si.specialInstructions) return { ...initialState, specialInstructions: si.specialInstructions, refinedInstructions: si.refinedInstructions };
    } catch {}
    return initialState;
  });
  ...
  }, [state, kbSelectedOffer, selectedBenefitsByOffer, activePanelId, userLogo, autoAccentHex, logoMode, logoWasUploaded, themeMode, donePanels]);
  ...
  function handlePanel2Done() {
    if (!kbSelectedOffer) { setPanel2DoneError("Select an offer first."); return; }
    if (selectedBenefitsSet(kbSelectedOffer).size < 3) { setPanel2DoneError("Select at least 3 benefits to continue."); return; }
  ...
  function generate() {
    // Benefit selection validation. Only applies when an offer is chosen
    // (no offer = no benefit panel to choose from = existing path unchanged).
    let orderedSelectedBenefits: string[] = [];
    if (kbSelectedOffer) {
      const sourceBenefits = kbBenefitsByOffer[kbSelectedOffer] ?? [];
      const picked = selectedBenefitsByOffer[kbSelectedOffer] ?? new Set<string>();
      if (sourceBenefits.length > 0 && picked.size === 0) {
        setGenerateError("Please select at least one benefit statement.");
        return;
      }
      // Preserve original chip order — filter the source array by membership.
      orderedSelectedBenefits = sourceBenefits.filter((b) => picked.has(b));
    }
    setGenerateError(null);
    setGenerated(
      buildPrompt(
        state,
        {
          selectedOffer: kbSelectedOffer,
          benefits: orderedSelectedBenefits,
        },
        undefined,
  ...
  const doneButtonShouldPulse =
    activePanelId === "offer-benefits" &&
    !!kbSelectedOffer &&
    selectedBenefitsSet(kbSelectedOffer).size >= 3;
        ...
        <aside className="builder-panel">
          ...
          <WorkflowPanel
            ...
            onDone={() => {
              ...
              const p2Done = !!kbSelectedOffer && selectedBenefitsSet(kbSelectedOffer).size >= 3;
              if (p2Done) {
                setActivePanelId("");
              } else {
                setPanel6DoneError("Almost there — select an offer and 3+ benefits in Step 2 to generate.");
                setPanel2IncomingAttention(true);
                setActivePanelId("offer-benefits");
              }
            }}
        ...
        <div className="right-column">
          <section className={`kb-panel${activePanelId === "offer-benefits" ? " kb-step-active" : ""}${panel2IncomingAttention ? " panel2-incoming-attention" : ""}`}>
            <header className="kb-panel-header">
              ...
              <p>Pick one offer, then 3+ benefits.</p>
              ...
              {(showSubOffers
                ...
                <div className="kb-chips">
                  {(showSubOffers
                    ...
                    <div
                      key={offer}
                      className={`kb-chip ${kbSelectedOffer === offer ? "active" : ""} ${kbSubOffers.includes(offer) && !kbOffers.includes(offer) ? "kb-chip-sub" : ""}`}
                      onClick={() => editingOffer !== offer && setKbSelectedOffer(offer)}
                    >
                      {editingOffer === offer ? (
                        <input
                          autoFocus
              ...
              <div className="kb-subcard-head">
                ...
                {kbSelectedOffer && kbBenefitsByOffer[kbSelectedOffer]?.length ? (
                  <span className="kb-benefit-counter">
                    {selectedBenefitsSet(kbSelectedOffer).size} / {BENEFIT_SELECTION_CAP} selected
                  </span>
                ) : null}
              </div>
              {kbSelectedOffer && kbBenefitsByOffer[kbSelectedOffer]?.length ? (
